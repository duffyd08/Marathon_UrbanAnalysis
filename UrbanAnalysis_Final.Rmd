...
##Creating Race Weekend Cols binary numeric in Data Frames
```{r}
all_boston = all_boston%>%
  mutate(race_weekend = ifelse(date %in% as.Date(c("2024-04-12", "2024-04-13", "2024-04-14","2024-04-15")), 1, 0))

all_chicago = all_chicago%>%
  mutate(race_weekend = ifelse(date %in% as.Date(c("2025-10-11", "2025-10-12", "2025-10-13")), 1, 0))

all_denver = all_denver%>%
  mutate(race_weekend = ifelse(date %in% as.Date(c("2024-05-17", "2024-05-18", "2024-05-19")), 1, 0))

all_ny = all_ny%>%
  mutate(race_weekend = ifelse(date %in% as.Date(c("2024-11-01", "2024-11-02", "2024-11-03")), 1, 0))

all_sf = all_sf%>%
  mutate(race_weekend = ifelse(date %in% as.Date(c("2024-07-26", "2024-07-27", "2024-07-28")), 1, 0))
```

```{r}
table(all_boston$race_weekend)

table(all_chicago$race_weekend)

table(all_denver$race_weekend)

table(all_ny$race_weekend)

table(all_sf$race_weekend)
```

##Seeing if Race weekend has an impact on price for Boston
```{r}
Boston_Price_Race_Weekend = lm(price ~ race_weekend, data = all_boston)
summary(Boston_Price_Race_Weekend)
```

##Seeing if Race weekend has an impact on price for Chicago
```{r}
Chicago_Price_Race_Weekend = lm(price ~ race_weekend, data = all_chicago)
summary(Chicago_Price_Race_Weekend)
```

##Seeing if Race weekend has an impact on price for Denver
```{r}
Denver_Price_Race_Weekend = lm(price ~ race_weekend, data = all_denver)
summary(Denver_Price_Race_Weekend)
```

##Seeing if Race weekend has an impact on price for New York
```{r}
NY_Price_Race_Weekend = lm(price ~ race_weekend, data = all_ny)
summary(NY_Price_Race_Weekend)
```

##Seeing if Race weekend has an impact on price for San Francisco
```{r}
SF_Price_Race_Weekend = lm(price ~ race_weekend, data = all_sf)
summary(SF_Price_Race_Weekend)
```

##Grouping by Neighborhood and Race Weekend
```{r}
Average_Price_Neighborhood_Boston_Weekend = all_boston%>%
  group_by(neighbourhood_cleansed, race_weekend)%>%
  summarize(Average_Price = mean(price, na.rm = TRUE), .groups = "drop")

Average_Price_Neighborhood_Chicago_Weekend = all_chicago%>%
  group_by(neighbourhood_cleansed, race_weekend)%>%
  summarize(Average_Price = mean(price, na.rm = TRUE), .groups = "drop")

Average_Price_Neighborhood_Denver_Weekend = all_denver%>%
  group_by(neighbourhood_cleansed, race_weekend)%>%
  summarize(Average_Price = mean(price, na.rm = TRUE), .groups = "drop")

Average_Price_Neighborhood_NY_Weekend = all_ny%>%
  group_by(neighbourhood_cleansed, race_weekend)%>%
  summarize(Average_Price = mean(price, na.rm = TRUE), .groups = "drop")

Average_Price_Neighborhood_SF_Weekend = all_sf%>%
  group_by(neighbourhood_cleansed, race_weekend)%>%
  summarize(Average_Price = mean(price, na.rm = TRUE), .groups = "drop")
```


##Seeing if Race Weekend Has an Impact on Average Price by Neighborhood Boston
```{r}
boston_neighborhood_price_race_weekend = lm(Average_Price ~ race_weekend, data = Average_Price_Neighborhood_Boston_Weekend)

summary(boston_neighborhood_price_race_weekend)
```

##Seeing if Race Weekend Has an Impact on Average Price by Neighborhood Chicago
```{r}
chicago_neighborhood_price_race_weekend = lm(Average_Price ~ race_weekend, data = Average_Price_Neighborhood_Chicago_Weekend)

summary(chicago_neighborhood_price_race_weekend)
```

##Seeing if Race Weekend Has an Impact on Average Price by Neighborhood Denver
```{r}
denver_neighborhood_price_race_weekend = lm(Average_Price ~ race_weekend, data = Average_Price_Neighborhood_Denver_Weekend)

summary(denver_neighborhood_price_race_weekend)
```

##Seeing if Race Weekend Has an Impact on Average Price by Neighborhood New York
```{r}
ny_neighborhood_price_race_weekend = lm(Average_Price ~ race_weekend, data = Average_Price_Neighborhood_NY_Weekend)

summary(ny_neighborhood_price_race_weekend)
```

##Seeing if Race Weekend Has an Impact on Average Price by Neighborhood San Francisco
```{r}
sf_neighborhood_price_race_weekend = lm(Average_Price ~ race_weekend, data = Average_Price_Neighborhood_SF_Weekend)

summary(sf_neighborhood_price_race_weekend)
```

##Race Weekends and Availability (Adding race weekend into total listings df's)
```{r}
#Boston
Average_Total_Listings_Race_Weekend_Boston = all_boston%>%
  mutate(date = as.Date(date), 
         available = as.numeric(available),
         race_weekend = ifelse(date %in% c("2024-04-12", "2024-04-13", "2024-04-14", "2024-04-15"), 1, 0))%>%
  group_by(date, race_weekend, listing_id)%>%
  summarize(Listings_Available = sum(available) > 0, .groups = "drop")%>% 
  group_by(date, race_weekend)%>%
  summarize(Average_Listings_Available = mean(Listings_Available), .groups = "drop")

#Chicago
Average_Total_Listings_Race_Weekend_Chicago = all_chicago%>%
  mutate(date = as.Date(date), 
         available = as.numeric(available),
         race_weekend = ifelse(date %in% c("2025-10-11", "2025-10-12", "2025-10-13"), 1, 0))%>%
  group_by(date, race_weekend, listing_id)%>%  
  summarize(Listings_Available = sum(available) > 0, .groups = "drop")%>% 
  group_by(date, race_weekend)%>%
  summarize(Average_Listings_Available = mean(Listings_Available), .groups = "drop")
...
#Denver
Average_Total_Listings_Race_Weekend_Denver = all_denver%>%
  mutate(date = as.Date(date), 
         available = as.numeric(available),
         race_weekend = ifelse(date %in% c("2024-05-17", "2024-05-18", "2024-05-19"), 1, 0))%>%
  group_by(date, race_weekend, listing_id)%>%  
  summarize(Listings_Available = sum(available) > 0, .groups = "drop")%>% 
  group_by(date, race_weekend)%>%
  summarize(Average_Listings_Available = mean(Listings_Available), .groups = "drop")
...
#New York
Average_Total_Listings_Race_Weekend_NY = all_ny%>%
  mutate(date = as.Date(date), 
         available = as.numeric(available),
         race_weekend = ifelse(date %in% c("2024-11-01", "2024-11-02", "2024-11-03"), 1, 0))%>%
  group_by(date, race_weekend, listing_id)%>%  
  summarize(Listings_Available = sum(available) > 0, .groups = "drop")%>% 
  group_by(date, race_weekend)%>%
  summarize(Average_Listings_Available = mean(Listings_Available), .groups = "drop")
...
#San Francisco
Average_Total_Listings_Race_Weekend_SF = all_sf%>%
  mutate(date = as.Date(date), 
         available = as.numeric(available),
         race_weekend = ifelse(date %in% c("2024-07-26", "2024-07-27", "2024-07-28"), 1, 0))%>%
  group_by(date, race_weekend, listing_id)%>%  
  summarize(Listings_Available = sum(available) > 0, .groups = "drop")%>% 
  group_by(date, race_weekend)%>%
  summarize(Average_Listings_Available = mean(Listings_Available), .groups = "drop")
...
##Boston Listings and Race Weekends Linear Regression
```{r}
Boston_listings_race_weekend = lm(Average_Listings_Available ~ race_weekend, data = Average_Total_Listings_Race_Weekend_Boston)

summary(Boston_listings_race_weekend)
```


##Chicago Listings and Race Weekends Linear Regression
```{r}
Chicago_listings_race_weekend = lm(Average_Listings_Available ~ race_weekend, data = Average_Total_Listings_Race_Weekend_Chicago)

summary(Chicago_listings_race_weekend)
```


##Denver Listings and Race Weekends Linear Regression
```{r}
Denver_listings_race_weekend = lm(Average_Listings_Available ~ race_weekend, data = Average_Total_Listings_Race_Weekend_Denver)

summary(Denver_listings_race_weekend)
```


##New York Listings and Race Weekends Linear Regression
```{r}
NY_listings_race_weekend = lm(Average_Listings_Available ~ race_weekend, data = Average_Total_Listings_Race_Weekend_NY)

summary(NY_listings_race_weekend)
```


##San Francisco Listings and Race Weekends Linear Regression
```{r}
SF_listings_race_weekend = lm(Average_Listings_Available ~ race_weekend, data = Average_Total_Listings_Race_Weekend_SF)

summary(SF_listings_race_weekend)
```
